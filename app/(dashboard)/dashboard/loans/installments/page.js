...
const toPayload = (state) => ({
  ...
  fineAmount: state.fineAmount === '' ? null : Number(state.fineAmount),
  repaymentStatus: state.repaymentStatus,
...
export default function LoanInstallmentsPage() {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await api.loanInstallments.list(params);
  ...
  const handleCreate = async () => {
    ...
    setInfo(null);
    try {
      await api.loanInstallments.create(toPayload(draft));
      setInfo('Created loan installment.');
      setShowCreate(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleUpdate = async () => {
    ...
    setInfo(null);
    try {
      await api.loanInstallments.update(selected.id, toPayload(draft));
      setInfo(`Updated installment ${selected.id}.`);
      setShowEdit(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleDelete = async () => {
    ...
    setInfo(null);
    try {
      await api.loanInstallments.remove(id);
      setInfo(`Deleted installment ${id}.`);
  ...
  const renderForm = () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        ...
        <input id="fineAmount" type="number" value={draft.fineAmount} onChange={(e) => setDraft((p) => ({ ...p, fineAmount: e.target.value }))} />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        <label htmlFor="dueAt">Due at</label>
        <input id="dueAt" value={draft.dueAt} onChange={(e) => setDraft((p) => ({ ...p, dueAt: e.target.value }))} placeholder="Timestamp" />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        <label htmlFor="repaymentStatus">Repayment status</label>
        <input id="repaymentStatus" value={draft.repaymentStatus} onChange={(e) => setDraft((p) => ({ ...p, repaymentStatus: e.target.value }))} />
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ fontWeight: 800, fontSize: '20px' }}>Loan Installments</div>
          <div style={{ color: 'var(--muted)' }}>Manage loan installments (due date, amount, status).</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} page={page} pageSize={size} onPageChange={setPage} emptyLabel="No installments found" />

      {showCreate && (
        <Modal title="Add loan installment" onClose={() => setShowCreate(false)}>
          {renderForm()}
      ...
      {showEdit && (
        <Modal title={`Edit installment ${selected?.id}`} onClose={() => setShowEdit(false)}>
          {renderForm()}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
      ...
      {confirmDelete && (
        <Modal title="Confirm delete" onClose={() => setConfirmDelete(null)}>
          <div style={{ color: 'var(--muted)' }}>
            Delete installment <strong>{confirmDelete.id}</strong>? This cannot be undone.
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
