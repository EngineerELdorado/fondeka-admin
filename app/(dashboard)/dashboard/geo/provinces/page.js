...
const emptyState = { name: '', countryId: '', active: true };

const toPayload = (state) => ({
  name: state.name,
  countryId: Number(state.countryId) || 0,
...
export default function ProvincesPage() {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await api.provinces.list(params);
  ...
  const handleCreate = async () => {
    ...
    setInfo(null);
    try {
      await api.provinces.create(toPayload(draft));
      setInfo('Created province.');
      setShowCreate(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleUpdate = async () => {
    ...
    setInfo(null);
    try {
      await api.provinces.update(selected.id, toPayload(draft));
      setInfo(`Updated province ${selected.id}.`);
      setShowEdit(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleDelete = async () => {
    ...
    setInfo(null);
    try {
      await api.provinces.remove(id);
      setInfo(`Deleted province ${id}.`);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ fontWeight: 800, fontSize: '20px' }}>Provinces</div>
          <div style={{ color: 'var(--muted)' }}>Provinces by country.</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} page={page} pageSize={size} onPageChange={setPage} emptyLabel="No provinces found" />

      {showCreate && (
        <Modal title="Add province" onClose={() => setShowCreate(false)}>
          {renderForm()}
      ...
      {showEdit && (
        <Modal title={`Edit province ${selected?.id}`} onClose={() => setShowEdit(false)}>
          {renderForm()}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
      ...
      {confirmDelete && (
        <Modal title="Confirm delete" onClose={() => setConfirmDelete(null)}>
          <div style={{ color: 'var(--muted)' }}>
            Delete province <strong>{confirmDelete.name || confirmDelete.id}</strong>? This cannot be undone.
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
