...
const emptyState = { name: '', provinceId: '', active: true };

const toPayload = (state) => ({
  name: state.name,
  provinceId: Number(state.provinceId) || 0,
...
export default function TerritoriesPage() {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await api.territories.list(params);
  ...
  const handleCreate = async () => {
    ...
    setInfo(null);
    try {
      await api.territories.create(toPayload(draft));
      setInfo('Created territory.');
      setShowCreate(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleUpdate = async () => {
    ...
    setInfo(null);
    try {
      await api.territories.update(selected.id, toPayload(draft));
      setInfo(`Updated territory ${selected.id}.`);
      setShowEdit(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleDelete = async () => {
    ...
    setInfo(null);
    try {
      await api.territories.remove(id);
      setInfo(`Deleted territory ${id}.`);
  ...
  const renderForm = () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        ...
        <input id="provinceId" type="number" value={draft.provinceId} onChange={(e) => setDraft((p) => ({ ...p, provinceId: e.target.value }))} />
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ fontWeight: 800, fontSize: '20px' }}>Territories</div>
          <div style={{ color: 'var(--muted)' }}>Territories by province.</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} page={page} pageSize={size} onPageChange={setPage} emptyLabel="No territories found" />

      {showCreate && (
        <Modal title="Add territory" onClose={() => setShowCreate(false)}>
          {renderForm()}
      ...
      {showEdit && (
        <Modal title={`Edit territory ${selected?.id}`} onClose={() => setShowEdit(false)}>
          {renderForm()}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
        ...
        <Modal title={`Details ${selected?.id}`} onClose={() => setShowDetail(false)}>
          <DetailGrid
            rows={[
              ...
              { label: 'Province ID', value: selected?.provinceId },
      ...
      {confirmDelete && (
        <Modal title="Confirm delete" onClose={() => setConfirmDelete(null)}>
          <div style={{ color: 'var(--muted)' }}>
            Delete territory <strong>{confirmDelete.name || confirmDelete.id}</strong>? This cannot be undone.
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
