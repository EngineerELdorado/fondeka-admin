...
const emptyState = { transactionId: '', senderAccountId: '', receiverAccountId: '' };

const toPayload = (state) => ({
  transactionId: Number(state.transactionId) || 0,
  senderAccountId: Number(state.senderAccountId) || 0,
  receiverAccountId: Number(state.receiverAccountId) || 0
});
...
export default function AccountTransfersPage() {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await api.accountTransfers.list(params);
  ...
  const handleCreate = async () => {
    ...
    setInfo(null);
    try {
      await api.accountTransfers.create(toPayload(draft));
      setInfo('Created account transfer.');
      setShowCreate(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleUpdate = async () => {
    ...
    setInfo(null);
    try {
      await api.accountTransfers.update(selected.id, toPayload(draft));
      setInfo(`Updated transfer ${selected.id}.`);
      setShowEdit(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleDelete = async () => {
    ...
    setInfo(null);
    try {
      await api.accountTransfers.remove(id);
      setInfo(`Deleted transfer ${id}.`);
  ...
  const renderForm = () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        ...
        <input id="senderAccountId" type="number" value={draft.senderAccountId} onChange={(e) => setDraft((p) => ({ ...p, senderAccountId: e.target.value }))} />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        <label htmlFor="receiverAccountId">Receiver account ID</label>
        <input id="receiverAccountId" type="number" value={draft.receiverAccountId} onChange={(e) => setDraft((p) => ({ ...p, receiverAccountId: e.target.value }))} />
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ fontWeight: 800, fontSize: '20px' }}>Account Transfers</div>
          <div style={{ color: 'var(--muted)' }}>Transfers between accounts with transaction IDs.</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} page={page} pageSize={size} onPageChange={setPage} emptyLabel="No account transfers found" />

      {showCreate && (
        <Modal title="Add account transfer" onClose={() => setShowCreate(false)}>
          {renderForm()}
      ...
      {showEdit && (
        <Modal title={`Edit transfer ${selected?.id}`} onClose={() => setShowEdit(false)}>
          {renderForm()}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
        ...
        <Modal title={`Details ${selected?.id}`} onClose={() => setShowDetail(false)}>
          <DetailGrid
            rows={[
              ...
              { label: 'Sender account', value: selected?.senderAccountId },
              { label: 'Receiver account', value: selected?.receiverAccountId },
      ...
      {confirmDelete && (
        <Modal title="Confirm delete" onClose={() => setConfirmDelete(null)}>
          <div style={{ color: 'var(--muted)' }}>
            Delete transfer <strong>{confirmDelete.id}</strong>? This cannot be undone.
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
