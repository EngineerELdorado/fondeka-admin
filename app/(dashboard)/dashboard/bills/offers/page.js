...
export default function BillOffersPage() {
  ...
  const handleDelete = async () => {
    ...
    setInfo(null);
    try {
      await api.billProductBillProviderOffers.remove(id);
      setInfo(`Deleted offer ${id}.`);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ fontWeight: 800, fontSize: '20px' }}>Product/Provider Offers</div>
          <div style={{ color: 'var(--muted)' }}>Manage offers per product-provider mapping.</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} emptyLabel="No offers found" />
      ...
      {showEdit && (
        <Modal title={`Edit offer ${selected?.id}`} onClose={() => setShowEdit(false)}>
          {renderForm()}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
      ...
      {confirmDelete && (
        <Modal title="Confirm delete" onClose={() => setConfirmDelete(null)}>
          <div style={{ color: 'var(--muted)' }}>
            Delete offer <strong>{confirmDelete.displayName || confirmDelete.name || confirmDelete.id}</strong>? This cannot be undone.
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
