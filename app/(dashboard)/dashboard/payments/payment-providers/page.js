...
export default function PaymentProvidersPage() {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await api.paymentProviders.list(params);
  ...
  const handleCreate = async () => {
    ...
    setInfo(null);
    try {
      await api.paymentProviders.create(toPayload(draft));
      setInfo('Created payment provider.');
      setShowCreate(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleUpdate = async () => {
    ...
    setInfo(null);
    try {
      await api.paymentProviders.update(selected.id, toPayload(draft));
      setInfo(`Updated payment provider ${selected.id}.`);
      setShowEdit(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleDelete = async () => {
    ...
    setInfo(null);
    try {
      await api.paymentProviders.remove(id);
      setInfo(`Deleted payment provider ${id}.`);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ fontWeight: 800, fontSize: '20px' }}>Payment Providers</div>
          <div style={{ color: 'var(--muted)' }}>Manage payment providers and ranking.</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} page={page} pageSize={size} onPageChange={setPage} emptyLabel="No payment providers found" />

      {showCreate && (
        <Modal title="Add payment provider" onClose={() => setShowCreate(false)}>
          {renderForm()}
      ...
      {showEdit && (
        <Modal title={`Edit payment provider ${selected?.id}`} onClose={() => setShowEdit(false)}>
          {renderForm()}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
      ...
      {confirmDelete && (
        <Modal title="Confirm delete" onClose={() => setConfirmDelete(null)}>
          <div style={{ color: 'var(--muted)' }}>
            Delete payment provider <strong>{confirmDelete.name || confirmDelete.id}</strong>? This cannot be undone.
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
