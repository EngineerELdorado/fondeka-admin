...
const emptyState = { accountId: '', savingProductId: '', balance: '', status: '' };
...
export default function SavingsPage() {
  ...
  const columns = useMemo(() => [
    ...
    { key: 'savingProductId', label: 'Product ID' },
  ...
  const openEdit = (row) => {
    ...
    setDraft({
      ...
      savingProductId: row.savingProductId ?? '',
      balance: row.balance ?? '',
      status: row.status ?? ''
    });
    setShowEdit(true);
  ...
  const renderForm = () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        <label htmlFor="savingProductId">Saving product ID</label>
        <input id="savingProductId" type="number" value={draft.savingProductId} onChange={(e) => setDraft((p) => ({ ...p, savingProductId: e.target.value }))} />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          ...
          <div style={{ color: 'var(--muted)' }}>Manage individual savings accounts.</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} emptyLabel="No savings found" />
        ...
        <Modal title={`Details ${selected?.id}`} onClose={() => setShowDetail(false)}>
          <DetailGrid
            rows={[
              ...
              { label: 'Saving product ID', value: selected?.savingProductId },
