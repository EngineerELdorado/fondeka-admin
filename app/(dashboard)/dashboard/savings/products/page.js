...
const emptyState = { name: '', interestRate: '', active: true };

const toPayload = (state) => ({
  name: state.name,
  interestRate: state.interestRate === '' ? null : Number(state.interestRate),
...
export default function SavingProductsPage() {
  ...
  const handleCreate = async () => {
    ...
    setInfo(null);
    try {
      await api.savingProducts.create(toPayload(draft));
  ...
  const handleUpdate = async () => {
    ...
    setInfo(null);
    try {
      await api.savingProducts.update(selected.id, toPayload(draft));
  ...
  const renderForm = () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        ...
        <input id="interestRate" type="number" value={draft.interestRate} onChange={(e) => setDraft((p) => ({ ...p, interestRate: e.target.value }))} />
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ fontWeight: 800, fontSize: '20px' }}>Saving Products</div>
          <div style={{ color: 'var(--muted)' }}>Configure saving products (rates, terms).</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} page={page} pageSize={size} onPageChange={setPage} emptyLabel="No saving products found" />

      {showCreate && (
        <Modal title="Add saving product" onClose={() => setShowCreate(false)}>
          {renderForm()}
      ...
      {showEdit && (
        <Modal title={`Edit saving product ${selected?.id}`} onClose={() => setShowEdit(false)}>
          {renderForm()}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
      ...
      {confirmDelete && (
        <Modal title="Confirm delete" onClose={() => setConfirmDelete(null)}>
          <div style={{ color: 'var(--muted)' }}>
            Delete saving product <strong>{confirmDelete.name || confirmDelete.id}</strong>? This cannot be undone.
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
