...
const emptyState = { savingId: '', amount: '', activityType: '', transactionId: '' };

const toPayload = (state) => ({
  savingId: Number(state.savingId) || 0,
...
export default function SavingActivitiesPage() {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await api.savingActivities.list(params);
  ...
  const handleCreate = async () => {
    ...
    setInfo(null);
    try {
      await api.savingActivities.create(toPayload(draft));
      setInfo('Created saving activity.');
      setShowCreate(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleUpdate = async () => {
    ...
    setInfo(null);
    try {
      await api.savingActivities.update(selected.id, toPayload(draft));
  ...
  const handleDelete = async () => {
    ...
    setInfo(null);
    try {
      await api.savingActivities.remove(id);
  ...
  const renderForm = () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        <label htmlFor="savingId">Saving ID</label>
        <input id="savingId" type="number" value={draft.savingId} onChange={(e) => setDraft((p) => ({ ...p, savingId: e.target.value }))} />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ fontWeight: 800, fontSize: '20px' }}>Saving Activities</div>
          <div style={{ color: 'var(--muted)' }}>Track deposits and withdrawals.</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} page={page} pageSize={size} onPageChange={setPage} emptyLabel="No saving activities found" />

      {showCreate && (
        <Modal title="Add saving activity" onClose={() => setShowCreate(false)}>
          {renderForm()}
        ...
        <Modal title={`Details ${selected?.id}`} onClose={() => setShowDetail(false)}>
          <DetailGrid
            rows={[
              ...
              { label: 'Saving ID', value: selected?.savingId },
