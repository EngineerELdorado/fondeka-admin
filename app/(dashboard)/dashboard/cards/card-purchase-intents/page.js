...
const emptyState = { cardProductId: '', transactionId: '' };
...
export default function CardPurchaseIntentsPage() {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await api.cardPurchaseIntents.list(params);
  ...
  const columns = useMemo(() => [
    { key: 'id', label: 'ID' },
    { key: 'cardProductId', label: 'Card product ID' },
  ...
  const handleCreate = async () => {
    ...
    setInfo(null);
    try {
      await api.cardPurchaseIntents.create(toPayload(draft));
      setInfo('Created card purchase intent.');
      setShowCreate(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleUpdate = async () => {
    ...
    setInfo(null);
    try {
      await api.cardPurchaseIntents.update(selected.id, toPayload(draft));
      setInfo(`Updated purchase intent ${selected.id}.`);
      setShowEdit(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleDelete = async () => {
    ...
    setInfo(null);
    try {
      await api.cardPurchaseIntents.remove(id);
      setInfo(`Deleted purchase intent ${id}.`);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ fontWeight: 800, fontSize: '20px' }}>Card Purchase Intents</div>
          <div style={{ color: 'var(--muted)' }}>Purchase intents linked to card transactions.</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} page={page} pageSize={size} onPageChange={setPage} emptyLabel="No card purchase intents found" />

      {showCreate && (
        <Modal title="Add card purchase intent" onClose={() => setShowCreate(false)}>
          {renderForm()}
      ...
      {showEdit && (
        <Modal title={`Edit purchase intent ${selected?.id}`} onClose={() => setShowEdit(false)}>
          {renderForm()}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
      ...
      {confirmDelete && (
        <Modal title="Confirm delete" onClose={() => setConfirmDelete(null)}>
          <div style={{ color: 'var(--muted)' }}>
            Delete purchase intent <strong>{confirmDelete.id}</strong>? This cannot be undone.
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
