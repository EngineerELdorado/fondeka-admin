...
const emptyState = { cardId: '', activityType: '', transactionId: '' };

const toPayload = (state) => ({
  cardId: Number(state.cardId) || 0,
...
export default function CardActivitiesPage() {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await api.cardActivities.list(params);
  ...
  const handleCreate = async () => {
    ...
    setInfo(null);
    try {
      await api.cardActivities.create(toPayload(draft));
      setInfo('Created card activity.');
      setShowCreate(false);
      fetchRows();
    } catch (err) {
      setError(err.message);
  ...
  const handleUpdate = async () => {
    ...
    setInfo(null);
    try {
      await api.cardActivities.update(selected.id, toPayload(draft));
  ...
  const handleDelete = async () => {
    ...
    setInfo(null);
    try {
      await api.cardActivities.remove(id);
  ...
  const renderForm = () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        ...
        <input id="cardId" type="number" value={draft.cardId} onChange={(e) => setDraft((p) => ({ ...p, cardId: e.target.value }))} />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        <label htmlFor="activityType">Activity type</label>
        <input id="activityType" value={draft.activityType} onChange={(e) => setDraft((p) => ({ ...p, activityType: e.target.value }))} placeholder="DEPOSIT or WITHDRAWAL" />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <div style={{ fontWeight: 800, fontSize: '20px' }}>Card Activities</div>
          <div style={{ color: 'var(--muted)' }}>Track deposits and withdrawals for cards.</div>
        </div>
      ...
      <DataTable columns={columns} rows={rows} page={page} pageSize={size} onPageChange={setPage} emptyLabel="No card activities found" />

      {showCreate && (
        <Modal title="Add card activity" onClose={() => setShowCreate(false)}>
          {renderForm()}
