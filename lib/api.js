...
const accounts = {
  ...
  listWithCustomFees: (query) => request(withQuery('/accounts/with-custom-fees', query)),
  ...
  getFiatWallets: (id) => request(`/accounts/${id}/fiat-wallets`),
  loanServiceRestrictions: {
    list: (query) => request(withQuery('/accounts/loan-service-restrictions', query))
  },
  ...
  walletActions: {
    get: (accountId) => request(`/accounts/${accountId}/wallet-actions`),
    update: (accountId, payload) => request(`/accounts/${accountId}/wallet-actions`, { method: 'PUT', body: toBody(payload) }),
    remove: (accountId) => request(`/accounts/${accountId}/wallet-actions`, { method: 'DELETE' })
  },
...
const kycDocumentTypes = {
  list: (query) => request(withQuery('/kyc/document-types', query)),
  get: (id) => request(`/kyc/document-types/${id}`),
  create: (payload) => request('/kyc/document-types', { method: 'POST', body: toBody(payload) }),
  update: (id, payload) => request(`/kyc/document-types/${id}`, { method: 'PUT', body: toBody(payload) }),
  remove: (id) => request(`/kyc/document-types/${id}`, { method: 'DELETE' })
};

const enhancedKycVerifications = {
  list: (query) => request(withQuery('/enhanced-kyc-verifications', query)),
  get: (id) => request(`/enhanced-kyc-verifications/${id}`),
  createWithTarget: (path, payload, files = {}) => {
    const presentFiles = Object.entries(files).filter(([, file]) => Boolean(file));
    if (presentFiles.length > 0) {
      const formData = new FormData();
      formData.append('data', new Blob([JSON.stringify(payload)], { type: 'application/json' }));
      presentFiles.forEach(([key, file]) => formData.append(key, file));
      return request(path, { method: 'POST', body: formData });
    }
    return request(path, { method: 'POST', body: toBody(payload) });
  },
  createForAccount: (accountId, payload, files = {}) => {
    return enhancedKycVerifications.createWithTarget(`/enhanced-kyc-verifications/accounts/${accountId}`, payload, files);
  },
  createForEmail: (email, payload, files = {}) => {
    const params = new URLSearchParams({ email: String(email || '').trim() });
    return enhancedKycVerifications.createWithTarget(withQuery('/enhanced-kyc-verifications/accounts/by-email', params), payload, files);
  },
  review: (id, payload) => request(`/enhanced-kyc-verifications/${id}/review`, { method: 'PATCH', body: toBody(payload) }),
  remove: (id) => request(`/enhanced-kyc-verifications/${id}`, { method: 'DELETE' })
};

const enhancedKycDocumentRequirements = {
  list: (query) => request(withQuery('/kyc/enhanced-verification/document-requirements', query)),
  get: (id) => request(`/kyc/enhanced-verification/document-requirements/${id}`),
  create: (payload) => request('/kyc/enhanced-verification/document-requirements', { method: 'POST', body: toBody(payload) }),
  update: (id, payload) => request(`/kyc/enhanced-verification/document-requirements/${id}`, { method: 'PUT', body: toBody(payload) }),
  remove: (id) => request(`/kyc/enhanced-verification/document-requirements/${id}`, { method: 'DELETE' })
};

const transactions = {
  list: (query) => request(withQuery('/transactions', query)),
  get: (transactionId) => request(`/transactions/${transactionId}`),
  timeline: (transactionId) => request(`/transactions/${transactionId}/timeline`),
...
const featureFlags = {
  ...
  upsertOverrideByEmail: (key, email, payload) => {
    const params = new URLSearchParams({ email: String(email || '').trim() });
    return request(withQuery(`/feature-flags/${encodeURIComponent(key)}/overrides/by-email`, params), {
      method: 'PUT',
      body: toBody(payload)
    });
  },
  removeOverrideByEmail: (key, email) => {
    const params = new URLSearchParams({ email: String(email || '').trim() });
    return request(withQuery(`/feature-flags/${encodeURIComponent(key)}/overrides/by-email`, params), { method: 'DELETE' });
  }
};
...
const currencyCircuitBreaker = {
  get: () => request('/currency-circuit-breaker'),
  update: (payload) => request('/currency-circuit-breaker', { method: 'PUT', body: toBody(payload) })
};
...
const groupSavings = {
  ...
  members: {
    ...
    remove: (id, memberId, payload) =>
      request(`/group-savings/${id}/members/${memberId}/remove`, { method: 'POST', ...(payload ? { body: toBody(payload) } : {}) }),
...
const apiCrudResources = {
  ...
  fiatWalletActivities: crud('fiat-wallet-activities'),
  ...
  paymentMethodPaymentProviders: {
    ...crud('payment-method-payment-providers'),
    coverage: (query) => request(withQuery('/payment-method-payment-providers/coverage', query))
  },
  ...
  feeConfigs: {
    ...crud('fee-configs'),
    syncPawapay: (payload) => request('/payment-fee-configs/pawapay/sync', { method: 'POST', body: toBody(payload) })
  },
  ...
  billProductBillProviders: {
    ...
    previewCegawebCatalog: (id, payload) => request(`/bill-product-bill-providers/${encodeURIComponent(id)}/cegaweb-catalog/preview`, { method: 'POST', body: toBody(payload) }),
    syncCegawebCatalog: (id, payload) => request(`/bill-product-bill-providers/${encodeURIComponent(id)}/cegaweb-catalog/sync`, { method: 'POST', body: toBody(payload) })
  },
  ...
  paymentRequests: {
    ...
    walletSettlementPolicy: {
      get: () => request('/payment-requests/wallet-settlement-policy'),
      update: (payload) => request('/payment-requests/wallet-settlement-policy', { method: 'PUT', body: toBody(payload) })
    }
  },
  ...
  currencyProducts: {
    ...crud('currency-products'),
    getDefaultBankAccount: (id) => request(`/currency-products/${id}/bank-account-default`),
    updateDefaultBankAccount: (id, payload) => request(`/currency-products/${id}/bank-account-default`, { method: 'PUT', body: toBody(payload) }),
    removeDefaultBankAccount: (id) => request(`/currency-products/${id}/bank-account-default`, { method: 'DELETE' })
  },
  bankAccounts: {
    create: (payload) => request('/bank-accounts', { method: 'POST', body: toBody(payload) }),
    listOrders: (query) => request(withQuery('/bank-accounts/orders', query)),
    getOrder: (transactionId) => request(`/bank-accounts/orders/${transactionId}`),
    completeOrder: (transactionId, payload) =>
      request(`/bank-accounts/orders/${transactionId}/complete`, { method: 'POST', ...(payload ? { body: toBody(payload) } : {}) }),
    cancelOrder: (transactionId, payload) =>
      request(`/bank-accounts/orders/${transactionId}/cancel`, { method: 'POST', ...(payload ? { body: toBody(payload) } : {}) })
  },
  providerFxPolicies: crud('provider-fx-policies'),
  walletMigrations: {
    legacyUsdLatest: () => request('/wallet-migrations/legacy-usd/latest'),
    legacyUsdOverrides: (query) => request(withQuery('/wallet-migrations/legacy-usd/overrides', query))
  },
  ...
  fiatExchangeRates: {
    ...
    previewConversion: (payload) => request('/fiat-exchange-rates/preview-conversion', { method: 'POST', body: toBody(payload) })
  },
  cryptoQuotes,
  cryptoWallets: {
    ...crud('crypto-wallets'),
  ...
  fiatWallets: {
    ...crud('fiat-wallets'),
    activities: (walletId, query) => request(withQuery(`/fiat-wallets/${walletId}/activities`, query))
  },
...
const walletCurrencyAudit = {
  violations: (query) => request(withQuery('/wallet-currency-audit/violations', query)),
  reviewViolation: (transactionId, payload) =>
    request(`/wallet-currency-audit/violations/${encodeURIComponent(transactionId)}/review`, { method: 'PUT', body: toBody(payload) }),
  clearViolationReview: (transactionId) =>
    request(`/wallet-currency-audit/violations/${encodeURIComponent(transactionId)}/review`, { method: 'DELETE' })
};
...
const commerceStores = {
  list: (query) => request(withQuery('/commerce/stores', query)),
  updateVerification: (storeId, payload) =>
    request(`/commerce/stores/${encodeURIComponent(storeId)}/verification`, { method: 'PATCH', body: toBody(payload) }),
  restore: (storeId) => request(`/commerce/stores/${encodeURIComponent(storeId)}/restore`, { method: 'POST' })
};

const commerceSettlementPolicy = {
  get: () => request('/commerce/settlement-policy'),
  update: (payload) => request('/commerce/settlement-policy', { method: 'PUT', body: toBody(payload) }),
  getAccount: (accountId) => request(`/commerce/accounts/${encodeURIComponent(accountId)}/settlement-policy`),
  updateAccount: (accountId, payload) =>
    request(`/commerce/accounts/${encodeURIComponent(accountId)}/settlement-policy`, { method: 'PUT', body: toBody(payload) }),
  removeAccount: (accountId) => request(`/commerce/accounts/${encodeURIComponent(accountId)}/settlement-policy`, { method: 'DELETE' })
};

const commerceStoreCreationFee = {
  get: () => request('/commerce/store-creation-fee'),
  update: (payload) => request('/commerce/store-creation-fee', { method: 'PUT', body: toBody(payload) })
};
...
export const api = {
  ...
  kycs,
  kycDocumentTypes,
  enhancedKycVerifications,
  enhancedKycDocumentRequirements,
  transactions,
  caches,
  featureFlags,
  cronJobs,
  ...
  commerceStores,
  commerceSettlementPolicy,
  commerceStoreCreationFee,
