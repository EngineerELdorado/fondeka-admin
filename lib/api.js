...
const featureFlags = {
  ...
  upsertOverrideByEmail: (key, email, payload) =>
    request(withQuery(`/feature-flags/${encodeURIComponent(key)}/overrides/by-email`, { email }), { method: 'PUT', body: toBody(payload) }),
  removeOverrideByEmail: (key, email) =>
    request(withQuery(`/feature-flags/${encodeURIComponent(key)}/overrides/by-email`, { email }), { method: 'DELETE' })
};
